...
async fn core_jobs_register_host_set() {
    ...
    assert_eq!(reg.periodic.len(), 5, "Core host periodic jobs (audit-retention, mcp-health, artefact-cleanup, api-chat-cleanup, telephony-retention); checkpoint + moderation-retention are Enterprise");
