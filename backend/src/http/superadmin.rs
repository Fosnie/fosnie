...
const KNOBS: &[Knob] = &[
    ...
    Knob { key: "voice.silence_threshold_ms", label: "Voice turn silence (ms)", desc: "Live voice: trailing-silence the platform waits before ending the speaker's turn. The single highest-leverage latency dial — tune in 100 ms steps. Lower = snappier but risks cutting the user off; higher = feels slow. Generous by default because without semantic turn detection the silence gate alone ends a turn, and a natural mid-thought pause must not chop the speaker.", value_type: ConfigValueType::Int, default: "1500", min: Some(200), max: Some(2000) },
    ...
    Knob { key: "voice.spec_debounce_ms", label: "Speculative search: minimum gap (ms)", desc: "Live voice: shortest interval between two speculative searches in one utterance.", value_type: ConfigValueType::Int, default: "700", min: Some(200), max: Some(5000) },
    Knob { key: "voice.spec_max_fires", label: "Speculative search: cap per turn", desc: "Live voice: hard ceiling on speculative searches in a single utterance, including the one fired at the soft endpoint. This is what bounds the cost of the feature.", value_type: ConfigValueType::Int, default: "3", min: Some(1), max: Some(10) },
    Knob { key: "voice.spec_soft_silence_pct", label: "Speculative search: soft endpoint (%)", desc: "Live voice: the pause at which the turn is probably ending, as a percentage of the turn-silence threshold above. At that point the transcript is nearly final, so the most valuable speculative search is made on the whole of it — this is the search that normally survives to answer the turn. Lower = earlier and more speculative.", value_type: ConfigValueType::Int, default: "50", min: Some(10), max: Some(90) },
    Knob { key: "voice.spec_eager_prob", label: "Speculative search: turn-completeness trigger", desc: "Live voice: turn-completeness confidence that also counts as a soft endpoint, so the semantic detector can trigger the search before the pause is long enough. Requires semantic turn detection above; 1.0 leaves the soft-endpoint percentage in sole charge. 0–1; the range is not enforced on write (a fractional knob) — values outside it are clamped when the setting is read.", value_type: ConfigValueType::Float, default: "0.4", min: None, max: None },
    Knob { key: "voice.spec_timeout_secs", label: "Speculative search: deadline", desc: "Live voice: wall-clock budget for one speculative search. Far tighter than a turn's own retrieval deadline by design — a speculation that has not landed shortly after the speaker stops has missed its purpose. On a very large corpus this can expire where the turn's own search would have succeeded; the turn then searches normally.", value_type: ConfigValueType::Int, default: "12", min: Some(3), max: Some(60) },
    Knob { key: "voice.spec_reuse_jaccard", label: "Speculative search: reuse similarity", desc: "Live voice: how much of their wording a speculative query and the finished sentence must share for the speculative result to answer the turn. Catches the case where the speaker re-ordered their question rather than finishing it. Raise to be stricter (more searches discarded, more turns pay full retrieval cost). 0–1; the range is not enforced on write (a fractional knob) — values outside it are clamped when the setting is read.", value_type: ConfigValueType::Float, default: "0.7", min: None, max: None },
    Knob { key: "voice.spec_reuse_new_ratio", label: "Speculative search: reuse tail allowance", desc: "Live voice: when the speculative query turns out to be the opening of the finished sentence, the largest share of that sentence that may be words the search never saw. Lower = stricter. 0–1; the range is not enforced on write (a fractional knob) — values outside it are clamped when the setting is read.", value_type: ConfigValueType::Float, default: "0.35", min: None, max: None },
    ...
    Knob { key: "desktop.download_url", label: "Desktop download page", desc: "Where a user is sent to get the desktop client when this installation serves no installer of its own. Upload one here and the button hands out the local copy instead, so nobody has to reach an external site.", value_type: ConfigValueType::String, default: "https://get.fosnie.dev/desktop/", min: None, max: None },
];
