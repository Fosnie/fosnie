...
pub enum ServerFrame {
    ...
    /// The full list is sent each update (replace, don't merge).
    ...
    /// Pushed once per occurrence, shortly before it is due.
    ...
    #[serde(rename = "pong")]
    Pong,
    /// Any frame type the reader does not know.
    ///
    /// The server never sends this: it exists for the readers. A client built
    /// against one release and pointed at a newer instance meets frames that did
    /// not exist when it was compiled, and the honest response is to ignore them
    /// and keep the connection — the same courtesy [`ClientFrame::Unknown`]
    /// extends in the other direction.
    #[serde(other)]
    Unknown,
}

/// One step in a `track_steps` checklist. Status ∈ pending | running |
/// done | skipped.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct StepOut {
    pub title: String,
...
///
/// Every field defaults on the way in: a reader compiled against an older
/// release meets citations from a newer one, and dropping the whole frame over a
/// field it has never heard of would cost the user their sources.
...
fn envelope<T: Serialize>(frame: &T) -> String {
    serde_json::to_string(&Envelope { version: PROTOCOL_VERSION, frame })
...
impl ClientFrame {
    /// The frame as it goes on the wire, version included — the mirror of
    /// [`ServerFrame::to_json`], for the clients that are not the web
    /// application and so build their frames in Rust.
    pub fn to_json(&self) -> String {
        envelope(self)
    }
}
